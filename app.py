...
training_log_cps = pd.read_csv(
    "calibration_log.csv.gz", compression="gzip"
)
...
total_loss_log = training_log[
    (training_log.name == name) & (training_log.time_period == year)
]

ground_truth = total_loss_log[
    total_loss_log["Source dataset"] == "Official"
].value.values[0]
last_value = total_loss_log[
    total_loss_log["Source dataset"] == "Enhanced FRS"
].value.values[-1]
...
last_value_df = (
    training_log[training_log.name == name]
    .groupby(["Source dataset", "time_period"])
    .last()
    .reset_index()
)
...
last_value_df = (
    training_log.groupby(["Source dataset", "time_period", "name"])
    .last()
    .reset_index()
)
...
if show_full_table:
    comparison_df = pd.DataFrame(
        {
            "Official": last_value_df[
                last_value_df["Source dataset"] == "Official"
            ].value.values,
            "Enhanced FRS": last_value_df[
                last_value_df["Source dataset"] == "Enhanced FRS"
            ].value.values,
            "Year": last_value_df[
                last_value_df["Source dataset"] == "Official"
            ].time_period.values,
            "Metric": last_value_df[
                last_value_df["Source dataset"] == "Official"
            ].name.values,
        }
    )
    comparison_df["Relative error"] = (
        comparison_df["Enhanced FRS"] - comparison_df["Official"]
    ) / comparison_df["Official"]
    comparison_df["Absolute error"] = (
        comparison_df["Enhanced FRS"] - comparison_df["Official"]
    )
    comparison_df["Absolute relative error"] = np.abs(
        comparison_df["Relative error"]
    )
    st.write(
